...
export function ScadenzarioRevisioni() {
  ...
  const handleWhatsApp = (telefono) => {
    const message = 'Scadenza revisione imminente! Controlla la tua data di scadenza revisione';
...
//fetch per prendere i dati della revisione
const fetchscadRev = async () => {
    ...
    try {
        ...
        if (filtroConferma === 1) { // Non confermate
            scadRevQuery = query(
                scadRevCollection,
                where("scadenzario.revisione.dataScadenza", ">=", startOfMonth),
                where("scadenzario.revisione.dataScadenza", "<=", endOfMonth),
                where("scadenzario.revisione.conferma", "==", false),
                orderBy("scadenzario.revisione.dataScadenza", "asc"),
                limit(100)
            );
            ...
            scadRevQuery = query(
                ...
                where("scadenzario.revisione.dataScadenza", ">=", startOfMonth),
                where("scadenzario.revisione.dataScadenza", "<=", endOfMonth),
                where("scadenzario.revisione.conferma", "==", true),
                orderBy("scadenzario.revisione.dataScadenza", "asc"),
                limit(100)
            );
        ...
        const workCardList = await Promise.all(
            scadRevnapshot.docs.map(async (doc) => {
                ...
                return {
                    id: doc.id,
                    ...data,
                    dataScadenzaRevisione: data.scadenzario.revisione.dataScadenza,
                    username: customerData.username,
                    telefono: customerData.telefono,
                    conferma: data.scadenzario.revisione.conferma, 
                    customerData,
                };
            })
        );
  ...
  useEffect(() => {
    fetchscadRev(); // Fetch iniziale senza ricerca
  }, []);
//--------------------------------------------------
...
const handleConfermaChange = async (id, newValue) => {
    try {
        ...
        await updateDoc(scadRevDoc, {
            "scadenzario.revisione.conferma": newValue,
        });
      ...
      >
        <div className="container-fluid">
          <div className="d-flex justify-content-between align-items-center">
            <h2 className="mb-0 titlePage">Scadenzario Revisioni</h2>
          </div>
          ...
          <div className="d-flex align-items-center justify-content-between mt-4">
            ...
            <div className="d-flex align-items-center">
              <IconButton variant="contained" onClick={() => {fetchscadRev()}}>
                <RefreshIcon/>
              </IconButton>
            </div>
          </div>
